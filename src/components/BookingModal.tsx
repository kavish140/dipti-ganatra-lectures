...
import { Calendar, Clock, MapPin, CheckCircle2, IndianRupee } from 'lucide-react';
...
const BookingModal = ({ lecture, defaultPrice, open, onClose }: BookingModalProps) => {
      ...
      <DialogContent className="sm:max-w-lg">
        {success ? (
          <div className="flex flex-col items-center gap-4 py-8 text-center">
            ...
            {lecturePrice && (
              <p className="text-sm text-muted-foreground">
                Lecture fee: <strong>INR {formatRupees(lecturePrice)}</strong>
              </p>
            )}
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="flex flex-wrap gap-4 pt-2 text-xs text-muted-foreground">
                ...
                {lecturePrice && (
                  <span className="flex items-center gap-1"><IndianRupee size={13} /> INR {formatRupees(lecturePrice)}</span>
                )}
              </DialogDescription>
            </DialogHeader>
